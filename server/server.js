...
app.post('/api/artworks', async (req, res) => {
  try {
    const { title, medium, year, img, description, has360 } = req.body;
    ...
    const newArtwork = new Artwork({ title, medium, year, img, description, has360, order: count });
...
app.patch('/api/artworks/:id', async (req, res) => {
  try {
    ...
    const updated = await Artwork.findByIdAndUpdate(
      req.params.id,
      { title, medium, year, img, description, has360 },
      { new: true }
    );
